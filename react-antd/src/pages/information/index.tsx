...
const InformationContent = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'createdAt',
      key: 'createdAt',
      render: (createdAt: string) => <span>{dayjs(createdAt).format('YYYY-MM-DD HH:mm:ss')}</span>,
    },
    {
      title: '更新时间',
      dataIndex: 'updatedAt',
      key: 'updatedAt',
      render: (updatedAt: string) => <span>{dayjs(updatedAt).format('YYYY-MM-DD HH:mm:ss')}</span>,
    },
    {
      title: '操作',
      key: 'action',
  ...
  const fetchData = () => {
    ...
    api
      .post(apiConfig.Information.getall + '?page=' + page + '&pageSize=' + pageSize, {
        filters,
      })
      .then(res => {
...
const Information = () => {
  const pathname = location.pathname.split('/')[1];
  return <InformationContent key={pathname} />;
};
